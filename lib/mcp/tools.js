...
export async function tagNote(supabase, ownerId, args) {
  const { id, tag_name } = args;

  const tagId = await resolveTagIdByName(supabase, ownerId, tag_name);
  if (!tagId) throw new Error(`Tag "${tag_name}" not found`);

  const { error } = await supabase.from('note_tags').upsert({ note_id: id, tag_id: tagId });
  if (error) throw new Error(error.message);

  return JSON.stringify({ id, tag_name, tagged: true }, null, 2);
}

export async function tagTodo(supabase, ownerId, args) {
  const { id, tag_name } = args;

  const tagId = await resolveTagIdByName(supabase, ownerId, tag_name);
  if (!tagId) throw new Error(`Tag "${tag_name}" not found`);

  const { error } = await supabase.from('todo_tags').upsert({ todo_id: id, tag_id: tagId });
  if (error) throw new Error(error.message);

  return JSON.stringify({ id, tag_name, tagged: true }, null, 2);
}

export async function createTag(supabase, ownerId, args) {
  const { name, type = 'project' } = args;
  const id = uuidv4();

  const { data: maxPosRows } = await supabase
    .from('tags').select('position').eq('owner_id', ownerId)
    .order('position', { ascending: false }).limit(1);
  const position = maxPosRows?.length > 0 && maxPosRows[0].position !== null
    ? maxPosRows[0].position + 1 : 0;

  const { data, error } = await supabase.from('tags')
    .insert({ id, owner_id: ownerId, name, type, position })
    .select('id, name, type')
    .single();
  if (error) throw new Error(error.message);
